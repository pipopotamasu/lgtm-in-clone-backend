import request from "supertest";
import app from "@src/app";
import { Post } from "@models/Post";
import { PostBookmark } from "@models/PostBookmark";
import { login } from "@test/helpers/user";
import { Types } from "mongoose";

describe("Get /api/v1/posts/random", () => {
  describe("errors", () => {
    describe("post does not exits", () => {
      describe("fetch by ObjectId", () => {
        it("returns not found error", async () => {
          const res = await request(app).get("/api/v1/posts/random").expect(404);
          expect(res.body.errors[0]).toBe("Not found.");
        });
      });
    });
  });

  describe("post exists", () => {
    it("is successfull get post", async () => {
      const post = await Post.create({ src: "path/to/src", userId: "testuserid" });
      const res = await request(app).get("/api/v1/posts/random").expect(200);

      expect(res.body).toEqual(post.response());
    });

    describe("post has bookmark which was created by myself", () => {
      it("is successfull get post", async () => {
        const { loginCookie, user } = await login();
        const post = new Post({ src: "path/to/src", userId: "testuserid" });
        const bookmark = await PostBookmark.create({ postId: post.id, userId: user.id });
        post.bookmarks.push(bookmark);
        await post.save();

        const res = await request(app).get("/api/v1/posts/random").set("Cookie", [loginCookie]).expect(200);

        expect(res.body.bookmarked).toBe(true);
      });
    });

    describe("post has bookmark which was created by other user", () => {
      it("is successfull get post", async () => {
        const { loginCookie } = await login();
        const post = new Post({ src: "path/to/src", userId: "testuserid" });
        const bookmark = await PostBookmark.create({ postId: post.id, userId: Types.ObjectId() });
        post.bookmarks.push(bookmark);
        await post.save();

        const res = await request(app).get("/api/v1/posts/random").set("Cookie", [loginCookie]).expect(200);

        expect(res.body.bookmarked).toBe(false);
      });
    });
  });
});
